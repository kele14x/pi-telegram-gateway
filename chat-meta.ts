...
export function parseChatMeta(raw: string): Map<number, ChatMetaEntry> {
  ...
  for (const [key, value] of Object.entries(data)) {
    ...
    if (typeof value === "string") entry.cwd = value;
    else if (typeof value?.cwd === "string") entry.cwd = value.cwd;
...
export function writeChatMeta(metaFile: string, entries: Map<number, ChatMetaEntry>): void {
  ...
  writeFileSync(tmp, serializeChatMeta(entries));
  renameSync(tmp, metaFile);
}

export function serializeChatMeta(entries: ReadonlyMap<number, ChatMetaEntry>): string {
